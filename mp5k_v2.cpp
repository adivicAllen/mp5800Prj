...
class AdivicTester : public TaskElement 
{
    public:
        bool MultiDUTEnable;
...
#ifdef OLD_CLIENT
        vector<AdivicTClient*> mp5k_list;
#else
        ...
        DWORD run(void)
        {
            ...
            do
            {
                ...
                while( !this->done && !Abort )
                {
                    // Get ID
				 
                    if ( mp5k_list.at( USER_ID_HANDLE ) != nullptr)
                    {
                        mp5k_list.at( USER_ID_HANDLE )->Send( "CONFigure:USER?" );
                        
						if( mp5k_list.at( USER_ID_HANDLE )->IsWaitTillReadToken( ackBuffer , "\n" ) ) 
						{
							if(  ackBuffer.length() > 2 )
							//if( SubString( ackBuffer, 2 ) )
							{
								ackBuffer = ackBuffer.substr( 2, ackBuffer.size() );
								uId = ( UINT )atoi( ackBuffer.c_str() );
								// Check ID
								if ( uId == gloMyID )
								{
									//Set Next ID
									sprintf_s( strBuffer , "CONFigure:USER %d" , gloNextID );
									mp5k_list.at( USER_ID_HANDLE )->Send( strBuffer );
									mp5k_list.at( USER_ID_HANDLE )->IsWaitTillReadToken( ackBuffer , "\n" );
								}
							}
						...
						{
#if MP5KLOG
								sprintf_s( strBuffer , "mp5k_Set_Next_ID:[failed] Data:%s",strBuffer);
    ...
    public:
        ...
        {
            boost::mutex::scoped_lock lock( itemMutex );
            cond.wait( lock );
        }
...
AdivicTask<AdivicTester>* Tester = nullptr;
...
/*GPS Member*/
AdivicTask<AdivicTester>* GPSTester = nullptr;
...
UINT mp5k_init( void )
{
	if( !isInit )
	{
		Tester = new AdivicTask< AdivicTester >( AdivicTester() );
		if ( !Tester )
		{
			// write to log
#if MP5KLOG
...
#endif
		Tester->task().mp5k_list.push_back( tmp ); // no use
		// default multi-dut variable
		Tester->task().MultiDUTEnable = false;
		// write to log
#if MP5KLOG
		sprintf_s( strBuffer , "mp5k_init:[MultiDUT = %d]", Tester->task().MultiDUTEnable );
		Mp5kLog.WriteLine( strBuffer );
#endif
		gloMyID = 1;
		gloNextID = 1;
...
UINT mp5k_add( const char* ip, UINT& handle )
{
	...
	{
		if( Tester->task().mp5k_list.at( handle )->IsConnected() )
		{
			return ::MP5K_OK;
		}
		Tester->task().mp5k_list.clear();
		ADIVIC_Sync_TelnetClient* temp = nullptr;
		Tester->task().mp5k_list.push_back( temp ); // no use
	}
...
// write to log
	...
	{
		int id = Tester->task().mp5k_list.size();
		Tester->task().mp5k_list.push_back( tmp );
		handle = id;
		strcpy_s(preIP,ip);
		Tester->task().mp5k_list.at( handle )->Send( VSA_INPUT_ON );
		if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
		{
// write to log
#if MP5KLOG
		...
		/*
		Tester->task().mp5k_list.at( handle )->Send( "INPut:TXQuality:SKIP:COUNt 5" );
		if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
		{
// write to log
#if MP5KLOG
  ...
  // allen 20170605
		...
		if( ID_TEMP->Open( ip ) )
		{
			USER_ID_HANDLE = Tester->task().mp5k_list.size();
			Tester->task().mp5k_list.push_back( ID_TEMP );
		}
		else
		{
			ID_TEMP->Close();
...
#if FOURPORT_CHECK
	Tester->task().mp5k_list.at( handle )->Send( "ROUTe:PORTs?" );
...
UINT mp5k_delete( const UINT& handle )
{
    ...
    if ( handle != NULL )
    {
        ...
        Tester->task().stop();
        Sleep(10);
        if ( Tester->task().mp5k_list.at( handle ) )
        {
#if MP5KLOG
            ...
            Tester->task().mp5k_list.at( handle )->Send( strBuffer );
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
            {
                return MP5K_REMOVE_ID_TIMEOUT;
            }

            Tester->task().mp5k_list.at( handle )->Close();

            Sleep(10);
            delete Tester->task().mp5k_list.at( handle );

#if MP5000API
			Tester->task().mp5k_list.at( USER_ID_HANDLE )->Close();

            Sleep(10);
            delete Tester->task().mp5k_list.at( USER_ID_HANDLE );
#endif
...
UINT mp5k_trigger_setting( const UINT& handle , const MP5K_TriggerConfig& trigger )
{
    ...
    Tester->task().gloTrigger = trigger;
    //
	 
    sprintf_s( strBuffer ,
               "CONFigure:TRIGger:TYPE %s;LEVel %.2f;OFFSet %.6f;SLICe %.8f" ,
               trigger_set[ Tester->task().gloTrigger.TriggerType ].c_str() ,
               Tester->task().gloTrigger.TriggerThreshold ,
               Tester->task().gloTrigger.TriggerOffset ,
               Tester->task().gloTrigger.TriggerGapTime
             );
    ...
    sprintf_s(strBuffer,
              ...
              handle ,
              trigger_set[ Tester->task().gloTrigger.TriggerType ].c_str() ,
              Tester->task().gloTrigger.TriggerThreshold , // dBm
              Tester->task().gloTrigger.TriggerGapTime * 1000000 , // us
              Tester->task().gloTrigger.TriggerOffset // us
             );
    Mp5kLog.WriteLine( strBuffer );
#endif
    return MP5K_OK;
...
UINT mp5k_correction_setting( const UINT& handle , const MP5K_CorrectionConfig& corr )
{
    ...
    Tester->task().gloCorrection = corr;
    sprintf_s( strBuffer,
               "CONFigure:OFDM:TRACking:FREQuency %s;:CONFigure:OFDM:TRACking:CHANnel %s" ,
               freq_est[ Tester->task().gloCorrection.FrequencyCorrection ].c_str() ,
               data_est[ Tester->task().gloCorrection.ChannelEstimation ].c_str()
             );
    Tester->task().mp5k_list.at( handle )->Send( strBuffer );
    if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
    {
...
UINT mp5k_xtaltrim_setting( const UINT& handle , const MP5K_XtalTrimConfig& trim )
{
    ...
    Tester->task().gloXtalTrimConfig = trim;
	...
	{
		if(port !=  Tester->task().gloXtalTrimConfig.UseRFPort)
		{
			OtherPort[cnt] = port;
			cnt++;
		}
	}
	Tester->task().mp5k_list.at( handle )->Send( "CONFigure:STANdard?" );
	...
	if( isFourPort )
	{
		sprintf_s( strBuffer ,
		           ...
		           standard,
				   Tester->task().gloXtalTrimConfig.UseRFPort ,
				   OtherPort[0] ,
				   OtherPort[1] ,
				   OtherPort[2] ,
		           Tester->task().gloXtalTrimConfig.FreqMHz ,
		           Tester->task().gloXtalTrimConfig.RBW ,
				   Tester->task().gloXtalTrimConfig.Trim_reference - Tester->task().gloXtalTrimConfig.Cableloss,
				   0.001
		         );
	}
	else
	...
	Tester->task().mp5k_list.at( handle )->Send( "INPut:SRATe 80Mhz" );
	if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
	{
    ...
    sprintf_s( strBuffer , "mp5k_xtaltrim_setting:[Handle = %d,Standard = GPRF,Port = %d,Freq(MHz) = %d,RBW(KHz) = %d]" ,
               ...
               Tester->task().gloXtalTrimConfig.UseRFPort ,
               Tester->task().gloXtalTrimConfig.FreqMHz ,
               Tester->task().gloXtalTrimConfig.RBW / 1000 // KHz
             );
    Mp5kLog.WriteLine( strBuffer );
#endif
    return MP5K_OK;
...
UINT mp5k_xtaltrim_getoffset( const UINT& handle , double& OffsetHz )
{
    ...
    // Check Sync
    Tester->task().mp5k_list.at( handle )->Send( strBuffer );
    Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" );
	...
	const double CheckPowerRef = Tester->task().gloXtalTrimConfig.Trim_reference - Tester->task().gloXtalTrimConfig.Cableloss  - 15.0;
...
UINT mp5k_cal_setting( const UINT& handle , const MP5K_CalConfig& cal )
{
    ...
    if ( Tester->task().gloCalConfig.Standard.IsGap )
    {
        if ( Tester->task().gloCalConfig.Standard.type != _802_11B )
        {
	...
	for(int port = 1 ;port < 5; port++)
	{
		if(port !=  Tester->task().gloCalConfig.UseRFPort)
		{
			OtherPort[cnt] = port;
			cnt++;
		}
	...
	if( isFourPort )
	{
		sprintf_s( strBuffer,
		           ...
		           standard ,
		           Tester->task().gloCalConfig.UseRFPort ,
				   OtherPort[0] ,
				   OtherPort[1] ,
				   OtherPort[2] ,
		           Tester->task().gloCalConfig.FreqMHz ,
		           Tester->task().gloCalConfig.RefLeveldBm
		         );
	}
	else
	{
	...
	if ( Tester->task().gloCalConfig.Standard.type != _802_11B )
    {
		if( Tester->task().gloCalConfig.Standard.OFDM.Modulation.Bandwidth == _80M )
		{
    ...
    sprintf_s( strBuffer ,"mp5k_cal_setting:[Handle = %d,Standard = %d,Port = %d,Freq(MHz) = %d,Ref level(dBm) = %.2f]" ,
               handle ,
               Tester->task().gloCalConfig.Standard.type,
               Tester->task().gloCalConfig.UseRFPort,
               Tester->task().gloCalConfig.FreqMHz,
               Tester->task().gloCalConfig.RefLeveldBm
             );
...
UINT mp5k_tx_setting( const UINT& handle , const MP5K_TxTestConfig& txtest )
{
    ...
    Tester->task().gloTXConfig = txtest;
	char standard[ 12 ] = {"/0"};


	if( Tester->task().gloTXConfig.isBeamforming )
	{
		MP5K_RxTestConfig rxconfig;
		rxconfig.FreqMHz = Tester->task().gloTXConfig.FreqMHz;
		rxconfig.OutPowerdBm = -60.00;
		rxconfig.PacketCount = 100;
		rxconfig.Standard = Tester->task().gloTXConfig.Standard;
		rxconfig.UseRFPort = Tester->task().gloTXConfig.UseRFPort;
	...
	if ( Tester->task().gloTXConfig.Standard.type != _802_11B )
    {
        sprintf_s( standard , "WIFI:OFDM;:" );
		isSameStandard = boost::equals("OFDM\n", ackBuffer );
    }
	...
	for(int port = 1 ;port < 5; port++)
	{
		if(port !=  Tester->task().gloTXConfig.UseRFPort)
		{
			OtherPort[cnt] = port;
			cnt++;
		}
	...
	if( isFourPort )
	{
		sprintf_s( strBuffer ,
		           ...
		           standard ,
		           Tester->task().gloTXConfig.UseRFPort ,
				   OtherPort[0] ,
				   OtherPort[1] ,
				   OtherPort[2] ,
		           Tester->task().gloTXConfig.FreqMHz ,
		           Tester->task().gloTXConfig.RefLeveldBm ,
		           Tester->task().gloTXConfig.CaptureLength
		         );
	}
	else
	{
    ...
    if ( Tester->task().gloTXConfig.Standard.type != _802_11B )
    {
		if( Tester->task().gloTXConfig.Standard.OFDM.Modulation.Bandwidth == _20M )
		{
			sprintf_s( strBuffer, "INPut:SRATe 80Mhz");
		}	
		else
	...
	if( !isSameStandard )
	{
		Tester->task().mp5k_list.at( handle )->Send( standard );	
		if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
		{
			return MP5K_STANDARD_ERROR;
		}
    ...
    Tester->task().mp5k_list.at( handle )->Send( "INPut:STARt" );
    if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
    {
    ...
    sprintf_s( strBuffer ,
               ...
               handle ,
               Tester->task().gloTXConfig.Standard.type,
               Tester->task().gloTXConfig.UseRFPort,
               Tester->task().gloTXConfig.FreqMHz,
               Tester->task().gloTXConfig.RefLeveldBm,
               Tester->task().gloTXConfig.CaptureLength * 1000 // ms
             );
    Mp5kLog.WriteLine( strBuffer );
#endif
	isWifi_TX_Sync = false;
...
UINT mp5k_tx_meas( const UINT& handle , MP5K_TxTestMeas& txmeas )
{
    ...
    Tester->task().mp5k_list.at( handle )->Send( strBuffer );
    bool IsSync = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Sync , "\n" );
    bool IsPower = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Power , "\n" );
	/* allen 20170620
	Tester->task().mp5k_list.at( handle )->Send( "CONFigure:TRIGger:TYPE IMMediate" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_TRIGGER_TYPE_ERROR;
    }
	*/
    if ( IsSync )
    {
        ...
        if ( retSync == -200 )
        {
			...
			Tester->task().mp5k_list.at( handle )->Send( CaptureCmd );
        ...
        else
        {
            ...
            Tester->task().mp5k_list.at( handle )->Send( strBuffer );
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Phase , "\n" ) )       // Phase Error
            {
                return MP5K_TX_READ_PHASE_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Frequency , "\n" ) )   // Frequency Error
            {
                return MP5K_TX_READ_FREQUENCY_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Symbol , "\n" ) )      // Symbol Clock Error
            {
                return MP5K_TX_READ_SYMBOL_CLOCK_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Lo , "\n" ) )          // Lo Leakage
            {
                return MP5K_TX_READ_LO_LEAKAGE_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( MASK , "\n" ) )        // MASK
            {
                return MP5K_TX_READ_MASK_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( EVM , "\n" ) )         // EVM
            {
            ...
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( MASKERROR , "\n" ) )       // Phase Error
            {
			...
			if( Tester->task().gloTXConfig.isBeamforming )
			{				
				Tester->task().mp5k_list.at( handle )->Send("FETCh:SEGMent1:OFDM:BEAMformed?");
				if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ISBeamformingON , "\n" ) )       // Phase Error
				{
					return MP5K_TX_READ_MASK_ERROR_ERROR;
				}
				...
				if (BFON == 1)
				{
					...
					Tester->task().mp5k_list.at( handle )->Send("CALCulate:TXQuality:ALL:SYNC;:FETCh:SEGMent1:OFDM:POWer?");
					IsSync = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Sync , "\n" );
					IsPower = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Power , "\n" );
					if ( IsSync || IsPower )
					{
						//Power = Power.substr(2 , Power.size() );
						SubString( Power, 2 );
...
UINT mp5k_rx_setting( const UINT& handle , const MP5K_RxTestConfig& rxtest )
{
    ...
    Tester->task().gloRXConfig = rxtest;
    UINT uTime = 0;
    // Standard
    char _standard[ 12 ];
    if ( Tester->task().gloRXConfig.Standard.type != _802_11B )
    {
    ...
    switch( Tester->task().gloRXConfig.Standard.type )
    {
    case _802_11AC:        sprintf_s( standard , "ac" );
    case _802_11N:
        if( Tester->task().gloRXConfig.Standard.type != _802_11AC )
        {
            sprintf_s( standard , "n" );
        }
        switch( Tester->task().gloRXConfig.Standard.OFDM.Modulation.Bandwidth )
        {
        ...
        // 11ac/n
        switch( Tester->task().gloRXConfig.Standard.OFDM.Modulation.rate )
        {
    ...
    case _802_11B:
        ...
        sprintf_s( bandwidth , "20" );
        switch( Tester->task().gloRXConfig.Standard.DSSS.rate )
        {
    ...
    if ( Tester->task().gloRXConfig.PacketCount < 0 )
    {
	...
	for(int port = 1 ;port < 5; port++)
	{
	if(port !=  Tester->task().gloRXConfig.UseRFPort)
		{
			OtherPort[cnt] = port;
			cnt++;
		}
	...
	if( isFourPort )
	{
		sprintf_s( strBuffer ,
		           ...
		           _standard ,
		           Tester->task().gloRXConfig.UseRFPort ,
				   OtherPort[0] ,
				   OtherPort[1] ,
				   OtherPort[2] ,
		           Tester->task().gloRXConfig.FreqMHz ,
		           Tester->task().gloRXConfig.OutPowerdBm ,
		           Tester->task().gloRXConfig.PacketCount ,
				   Path,
				   fileTemp.c_str()
		         );
	}
...
UINT mp5k_rx_start(const UINT& handle)
{
    ...
    if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" , rx_time_out ))
    {
        ...
        Tester->task().mp5k_list.at( handle )->Close();
        Tester->task().mp5k_list.at( handle )->Open( Tester->task().mp5k_list.at( handle )->ip , 5499 , 2 );
        // write to log
#if MP5KLOG
...
UINT mp5k_multi_dut( const bool& Enable )
{
    ...
    Tester->task().MultiDUTEnable = Enable;
    // write to log
#if MP5KLOG
...
UINT mp5k_wait_server_ready( const UINT& handle , const UINT& Milliseconds )
{
    ...
    if ( Tester->task().MultiDUTEnable )
    {
//        sprintf_s( strBuffer , "CONFigure:USER?" );
        Tester->task().mp5k_list.at( handle )->Send( "CONFigure:USER?" );
        if ( Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) == false )
        {
            // write to log
#if MP5KLOG
...
UINT mp5k_measure_done( const UINT& handle )
{
    ...
    if ( Tester->task().MultiDUTEnable )
    {
        ...
        Tester->task().Handle = handle;
        Tester->task().start_for_run();
...
UINT mp5k_tx_meas_extend( const UINT& handle , MP5K_TxTestMeasExtend& txmeasext )
{
	...
	if( Tester->task().gloTXConfig.Standard.type == _802_11AC && Tester->task().gloTXConfig.Standard.OFDM.Modulation.Bandwidth == _80M )
	{
		isWifi_TX_Sync = false;
		Tester->task().mp5k_list.at( handle )->Send( "CONFigure:OFDM:SPECtrum:WIDE ON" );
		if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
		{
	...
	{
		Tester->task().mp5k_list.at( handle )->Send( "CALCulate:TXQuality:ALL:SYNC" );
		IsSync = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Sync , "\n" );
	}
    ...
    if( IsSync )
    {
        ...
        else
        {
            ...
            Tester->task().mp5k_list.at( handle )->Send( strBuffer );
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( MARGIN , "\n",2 ) )
            {
                return MP5K_TXEX_READ_MARGIN_ERROR;
            }
            if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( FREQ , "\n", 2 ) )
            {
                return MP5K_TXEX_READ_FREQ_ERROR;
            }
            if ( Tester->task().gloTXConfig.Standard.type != _802_11B )
            {
                Tester->task().mp5k_list.at( handle )->Send( "FETCh:SEGMent1:OFDM:FLATness:CHECk?" );
                if( ! Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( FLATNESS , "0," , 2) )
                {
               ...
               //if( Tester->task().gloTXConfig.Standard.OFDM.Modulation.Bandwidth == _80M )
               //{
               ...
               //{
				...
				if( Tester->task().gloTXConfig.Standard.type == _802_11AC && Tester->task().gloTXConfig.Standard.OFDM.Modulation.Bandwidth == _80M )
				{
					Tester->task().mp5k_list.at( handle )->Send( "CONFigure:OFDM:SPECtrum:WIDE OFF" );
					if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
					{
						return MP5K_TX_SET_WIDE_ERROR;
					}
            ...
            {
                Tester->task().mp5k_list.at( handle )->Send( "FETCh:SEGMent1:DSSS:RAMP:ON:CHECK?");
                if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( RAMP_ON , "\n" ) )
                {
                    return MP5K_TXEX_READ_RAMP_ON_ERROR;
                }
				Tester->task().mp5k_list.at( handle )->Send( "FETCh:SEGMent1:DSSS:RAMP:OFF:CHECK?");
                if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( RAMP_OFF , "\n" ) )
                {
...
UINT mp5k_BT_tx_setting (const UINT& handle, const MP5K_BT_TxTestConfig& Setting)
{
    ...
    Tester->task().gloBtTXConfig = Setting;
	
	UINT OtherPort[3], cnt = 0;
	for(int port = 1 ;port < 5; port++)
	{
		if(port !=  Tester->task().gloBtTXConfig.UseRFPort)
		{
			OtherPort[cnt] = port;
			cnt++;
		}
 	...
 	if( isFourPort )
	{
		sprintf_s( strBuffer ,
            ...
            Tester->task().gloBtTXConfig.UseRFPort ,
			OtherPort[0] ,
			OtherPort[1] ,
			OtherPort[2] ,
            Tester->task().gloBtTXConfig.FreqMHz ,
            Tester->task().gloBtTXConfig.RefLeveldBm ,
            Tester->task().gloBtTXConfig.CaptureLength
            );
	}
	else
	{
	...
	if( !isSameStandard )
	{
		Tester->task().mp5k_list.at( handle )->Send( "BT" );	
		if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
		{
			return MP5K_STANDARD_ERROR;
		}
    ...
    sprintf_s( strBuffer ,
               ...
               handle ,
			   Tester->task().gloBtTXConfig.Type,
               Tester->task().gloBtTXConfig.UseRFPort,
               Tester->task().gloBtTXConfig.FreqMHz,
               Tester->task().gloBtTXConfig.RefLeveldBm,
               Tester->task().gloBtTXConfig.CaptureLength * 1000 // ms
             );
    Mp5kLog.WriteLine( strBuffer );
#endif
	isBT_TX_Sync = false;
...
UINT mp5k_BT_tx_meas(const UINT& handle, MP5K_BT_TxTestMeas& Bt_Tx_Meas)
{
	...
	if ( IsSync )
    {
        ...
        else
        {
			...
			switch( Tester->task().gloBtTXConfig.Type )
			{
			case DM1:
			case DH1:
			case DM3:
			...
			Tester->task().mp5k_list.at( handle )->Send( strBuffer );
			bool IsReturn = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( Return , "\n" );
			if( !IsReturn )
			{
#if MP5KLOG
                sprintf_s( strBuffer , "mp5k_BT_tx_meas:[Handle = %d,Power measurement timeout]" , handle );
			...
			std::vector<std::string> returnData;
			SubString(Return, 2);
			boost::split( returnData, std::string(Return) , boost::is_any_of(","));
...
UINT mp5k_BT_tx_meas_extend(const UINT& handle, MP5K_BT_TxTestMeasExtend& Bt_Tx_MeasExt)
{
	...
	if ( IsSync )
    {
        ...
        {
			Tester->task().mp5k_list.at( handle )->Send( "FETCh:SEGMent1:BT:ACPower?;ACPower:CHECk?" );
			bool IsACP_Return = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ACP_Return , "\n" );
			bool IsACP_Check_Return = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ACP_Check_Return , "\n" );
			if( !IsACP_Return )
			{
#if MP5KLOG
			...
			{
				std::vector<std::string> returnData;
				boost::split( returnData, std::string(ACP_Return) , boost::is_any_of(","));
...
UINT mp5k_BT_rx_setting(const UINT& handle, const MP5K_BT_RxTestConfig& Setting)
{
    ...
    Tester->task().gloBtRXConfig = Setting;
    UINT uTime = 0;	

	char	packetType[ BUFFER_LENGTH ];


	switch( Tester->task().gloBtRXConfig.Type )
    {
    ...
    sprintf_s( strBuffer ,
			   ...
			   Tester->task().gloBtRXConfig.UseRFPort ,
               Tester->task().gloBtRXConfig.FreqMHz ,
               Tester->task().gloBtRXConfig.OutPowerdBm,
			   Tester->task().gloBtRXConfig.BitCount ,
               packetType
             );
	*/
	  sprintf_s( strBuffer ,
	...
	*/
	sprintf_s( strBuffer ,"CONFigure:WAVE:GAP %f",  Tester->task().gloBtRXConfig.GapTime);
	Tester->task().mp5k_list.at( handle )->Send( strBuffer );
	if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ) )
	{
...
UINT mp5k_BT_rx_start(const UINT& handle)
{
	...
	Tester->task().mp5k_list.at( handle )->Send( "OUTPut:STARt" );
	// Check rx timeout
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" , BT_rx_time_out ))
	{
		//mp5k tester refresh or clear
...
UINT mp5k_VSA_AGC(const UINT& handle)
{
	...
	Tester->task().mp5k_list.at( handle )->Send( "INPut:LEVel:REFerence:AUTO" );
	
	if ( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" ) )
    {
...
UINT mp5k_GetSpectrumPower( const UINT& handle, double& StartFreq, double& OffsetFreq ,std::vector<float>& data )
{
    ...
    Tester->task().mp5k_list.at( handle )->Send( "CALCulate:TXQuality:ALL:SYNC;" );
    Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" );
	...
	if( !Tester->task().gloTXConfig.Standard.IsGap )
	{
		sprintf_s( strBuffer , "FETCh:SPECtrum:GPRF:DATA?" );
	}
	else if( Tester->task().gloTXConfig.Standard.type == _802_11B )
	{
	...
	if ( Tester->task().mp5k_list.at( handle )->IsWaitTillReadTokenForSpectrum( ackBuffer ,10240, "\n"  ))
	{
		data.clear();
...
UINT mp5k_VSA_SaveWaveform(const UINT& handle, const std::string& filename )
{
	...
	Tester->task().mp5k_list.at( handle )->Send( strBuffer );
	bool IsSync = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" );
	bool IsCaP = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" ) ;
...
UINT mp5k_VSA_LoadWaveform(const UINT& handle, const std::string& filename )
{
	...
	Tester->task().mp5k_list.at( handle )->Send( strBuffer );
	bool isLoad =  Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" ); 
	if( !isLoad )
	{
		return 999;
	}
...
/*	if ( Tester == nullptr)
	...
	Tester->task().mp5k_list.at( handle )->Send( pathCmd );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" ) )
	{
		return 999;
	}
	...
	for( int Count = 0 ; Count < TotalCount ; ++Count )
	{
		...
		Tester->task().mp5k_list.at( handle )->Send( buffer2 , totoalSize);
		IsSync = Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" );
	}
...
UINT mp5k_GPS_init( void )
{
	if( !isGPSInit )
	{
		...
		ADIVIC_Sync_TelnetClient* tmp = nullptr;		
		GPSTester->task().mp5k_list.push_back( tmp ); // no use
		// default multi-dut variable
		GPSTester->task().MultiDUTEnable = false;
		// write to log
#if MP5KLOG
		sprintf_s( strBuffer , "mp5k_GPS_init:[MultiDUT = %d]", GPSTester->task().MultiDUTEnable );
		Mp5kLog.WriteLine( strBuffer );
#endif
		gloMyID = 1;
		gloNextID = 1;
...
UINT mp5k_GPS_add( const char* ip , UINT&  handle)
{
	...
	if( 0 == strcmp(preIP,ip) )
	{
		if( GPSTester->task().mp5k_list.at( handle )->IsConnected() )
		{
			return ::MP5K_OK;
		}
		GPSTester->task().mp5k_list.clear();
		ADIVIC_Sync_TelnetClient* temp = nullptr;
		GPSTester->task().mp5k_list.push_back( temp ); // no use
	}
	int id = 0;
...
// write to log
	...
	{
		id = GPSTester->task().mp5k_list.size();
		GPSTester->task().mp5k_list.push_back( tmp );
		handle = id;
		strcpy_s(preIP,ip);
	}
// write to log
...
UINT mp5k_GPS_delete( const UINT& handle )
{
    ...
    if ( handle != NULL )
    {
        ...
        GPSTester->task().stop();
        Sleep(10);
        if ( GPSTester->task().mp5k_list.at( handle ) )
        {
#if MP5KLOG
			sprintf_s(strBuffer, "mp5k_GPS_delete:[Start Remove handle = %d]", handle);
			Mp5kLog.WriteLine( strBuffer );
#endif
            GPSTester->task().mp5k_list.at( handle )->Close();
            Sleep(10);
            delete GPSTester->task().mp5k_list.at( handle );
        }
        // write to log
#if MP5KLOG
...
UINT mp5k_GPS_SingleCh_Setting( const UINT& handle ,const GPS_SingleCh_Config& Setting)
{
    ...
    if ( handle != NULL )
    {
		...
		GPSTester->task().mp5k_list.at( handle )->Send( strBuffer );
		if ( !GPSTester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "0" , 200) )
		{
...
UINT mp5k_GPS_SingleCh_Start( const UINT& handle )
{
    ...
    if ( handle != NULL )
    {
		...
		GPSTester->task().mp5k_list.at( handle )->Send( strBuffer );
		if ( !GPSTester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "0" ) )
		{
...
UINT mp5k_GPS_CW_Signal( const UINT& handle )
{
    ...
    if ( handle != NULL )
    {
		...
		GPSTester->task().mp5k_list.at( handle )->Send( strBuffer );
		if ( !GPSTester->task().mp5k_list.at( handle )->IsWaitTillReadToken( strBuffer , "\n" ) )
		{
...
UINT mp5k_Cableloss_init( const UINT& handle )
{
	...
	Tester->task().mp5k_list.at( handle )->Send( "CONFigure:USER:REGister?" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
	...
	Tester->task().mp5k_list.at( handle )->Send( "OUTPut:POWer:LEVel -10" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
	
	Tester->task().mp5k_list.at( handle )->Send( "INPut:CAPTure:TIME  0.001" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
	...
	Tester->task().mp5k_list.at( handle )->Send( "GPRF" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
	
	Tester->task().mp5k_list.at( handle )->Send( "CONFigure:RFCorrection ON" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }

	Tester->task().mp5k_list.at( handle )->Send( "INPut:LEVel:REFerence -10" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
...
UINT mp5k_Cableloss_close( const UINT& handle)
{
	...
	Tester->task().mp5k_list.at( handle )->Send( "INPut:TXQuality:SKIP:COUNt 2" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
	
	Tester->task().mp5k_list.at( handle )->Send( "CONFigure:RFCorrection OFF" );
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( ackBuffer , "\n" ))
	{
        return MP5K_HANDLE_ERROR;
    }
...
UINT mp5k_Send_Cmd( const UINT& handle, std::string& cmd)
{
	...
	Tester->task().mp5k_list.at( handle )->Send( cmd );
...
UINT mp5k_Read_Cmd( const UINT& handle, std::string& data, const std::string& token)
{
	...
	if( !Tester->task().mp5k_list.at( handle )->IsWaitTillReadToken( data , token ))
	{
        return MP5K_HANDLE_ERROR;
    }
